...
class TemporaryDrawer extends React.Component {
  ...
  render() {
    ...
    const fullList = (
      <div>
        <List classes={{root: classes.root}} className={classes.noPadding} elevation={3}>
          {['about', 'media', 'merch', 'events', 'home'].map((text, index) => (
            <ListItem button key={text} className={classes.noPadding} >
    ...
    return (
      <div className="topNav">
        ...
        <IconButton onClick={this.toggleDrawer('top', true)} anchor="top" className={classes.menuButton} aria-label="Menu" elevation={3}>
            <MenuIcon />
        </IconButton>
        <Drawer
          anchor="top"
